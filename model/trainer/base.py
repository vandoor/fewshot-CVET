...
from utils import ensure_path, Averager, Timer, count_acc, compute_confidence_interval
...
class Trainer(object, metaclass=abc.ABCMeta):
    ...
    def try_logging(self, tl1, tl2, ta, tg=None):
        ...
        if self.train_step % args.log_interval == 0:
            ...
            print('data_timer: {:.2f} sec, forward_timer{:.2f} sec, backward_timer: {:.2f} sec, optim_timer: {:.2f} sec'
                  .format(self.dt.item(), self.ft.item(), self.bt.item(), self.ot.item())
            )
            self.logger.dump()
